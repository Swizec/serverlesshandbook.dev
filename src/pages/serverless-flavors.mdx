...
There's a bunch of providers out there with different features, different pricing, different DX, different focus ... how do you choose?
...
Did you know AWS was so big it's [more than half of Amazon's revenue](https://www.itproportal.com/news/aws-now-makes-up-over-half-of-all-amazon-revenue/)?

With over 165 services it's impossible to try or even know all of AWS. Here's a few I've found most useful:

- **EC2** – old school cloud. You get a virtual computer, set it up, and you're in control. Keeps running forever unless you make it stop.
- **S3** – the most popular solution for static files. Upload a file, get a URL, and the file stays there forever. No servers involved, can't host websites. Often used for image and video assets.
- **CloudFront** – a [CDN](https://en.wikipedia.org/wiki/Content_delivery_network) that integrates well with S3. Point users to static files via CloudFront and it serves files from the physically nearest server to your users. Caches aggressively. Your server gets less load, your users get faster apps.
- **IAM** – identity and account management. AWS forces you to use this one to manage permissions for everything else. It's super secure, tedious to set up, and afaik everyone hates it. Until it saves their butts.
- **Lambda** – the poster child for serverless. One of the first to popularize function-as-a-service architectures. This is what people usually mean by serverless. You write your function code, Lambda handles the rest.
- **SQS** – simple queue service is ... a simple way to implement queues. Best used for async communication between different services. Stores and retries messages if necessary.
...
AWS services add up fast once you build a real app or two. Every tool does one job and does it well. No single tool does *your* whole job.

### When to choose AWS

I like to use AWS when I need lots of control over both the developer experience and how everything fits together. 

For example I'd use AWS when a large part of my project involves data pipelines, coordinating between users, and other backend logic. The stuff you'd traditionally put on the backend.

You should use the serverless framework rather than AWS's UI for the most part. We'll talk about that in the [Serverless Developer eXperience](/serverless-dx) chapter.
...
Hosting those on AWS is difficult, too. Best use Netlify or Zeit I think. Or any other static site host you can find.
...
Best I can tell Azure's biggest draw is that it's Microsoft instead of Amazon. Lots of enterprises use it, but it hasn't quite captured the imagination of indie hackers.

Not sure why that is. Pricing sounds similar (cheap at first then expensive). Maybe it's a brand perception issue :)
...
Unlike with AWS and Azure, you get everything out of the box with Firebase. There's no setup, no configuration, nothing.
...
You have no control over the backend so your choice is to either change your app or rebuild everything from scratch.

## Netlify

[![](../images/netlify.png)](https://www.netlify.com/)

Netlify is wonderful. They invented everyone's favorite [JAMstack](https://en.wikipedia.org/wiki/Netlify#JAMstack) buzzword and focus on pushing the community forward. 

At first just a static site host for JavaScript-heavy applications they now offer some backend capabilities as well. 
...
- superb git integration for automated deploys
...
They've been around long enough to rely on and are still young enough that you'll get a proper response, if you have issues. I find that to be a great balance for my projects :)

If you like a git- and UI- first mentality for your deploys, Netlify is great.

### When not to choose Netlify

Netlify is almost always a great choice for your web app. 

Not sure I'd use them for backend code because their FaaS support is still new. There's also a concern I'd have about the level of control you get over backend functionality.
...
Zeit's focus, however, is a little different than Netlify's. They differ in approach as well.

Where Netlify makes static deploys as painless to configure and deploy via git and their UI, Zeit focuses on making *any* app as painless to deploy via their CLI as possible. Zeit is command-line first, git and UI second.

My favorite use-case for Zeit is buying domains from the command line and deploying static sites. 

Haven't used their backend support yet. They let you run Express apps which seems useful and there's even FaaS support nowadays.
...
Honestly it's a toss up between Netlify and Zeit right now. It seems they're both playing catch up. When one releases a new feature, the other is almost sure to get it a few months later.

I like Zeit's great command line interface and the fact I can call `now` in any project and it's gonna show up on the internet. Usually no clicking or config needed.
...
That means it's easier to get attention from the core team, even the founders. This, of course, is great, but can mean that the smaller team will take longer to resolve any really hairy issues you run into.

If you *prefer* interacting with UI and writing config files, then Zeit's focus might not be for you :)

## So ... what to choose?

My personal preference is to put the frontend on Netlify or Zeit and the backend on AWS's serverless ecosystem.
