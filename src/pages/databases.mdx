...
Serverless systems don't have a hard drive, so where do you keep your data? You need a database of some sort.

In this chapter, we're going to explore different database options and technologies, talk about their benefits, when to use them, and when they're a bad choice. 

Contrary to popular belief, there is no silver bullet. Each database has its own advantages and disadvantages. Aim to fit your database to your problem, not your problem to your database.
...
Every database technology or service should provide you with these features:

- keeps your data
- lets you query data
- lets you update data

*Keeping* data is the difference between a cache and a database. You can have an in-memory database for speed, but that doesn't make it a cache. Your data persists.
...
When you take a photo, your phone stores it as a file. Filenames often follow a naming convention with counting – `IMG_0001`, `IMG_0002`, etc. Some cameras add dates.
...
That's why images contain [EXIF metadata](https://en.wikipedia.org/wiki/Exif) – information about date, time, GPS location, even phone orientation and details about color correction. It's all there in the file in a structured way.
...
Here's how mine worked in the mid-2000's at the peak of the music piracy era. We all did it, it's fine, fight the system ✌️
...
Prefixing filenames with numbers allowed me to sort by filename and maintain song order. 😉
...
Encoding hierarchical information in filenames is also common. Every file is named according to some pattern – like `${artist}-${year}-${album}-${tracknumber}-${track}`.
...
## Using traditional databases – SQL
