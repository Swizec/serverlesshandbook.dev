...
Many GraphQL libraries add caching so you don't make the same calls too often. Some even consolidate queries so making 10 requests real fast gets wrapped into a single API call. 😲
...
Often touted as a replacement for REST, GraphQL is really a different approach to building an API. Better suited for many common use-cases, but comes with its own set of drawbacks.

Don't let the propaganda fool you 👉 you can and should use both REST and GraphQL. Most real world projects need both.

The main benefit of GraphQL is its declarative nature.
...
So while REST queries can be declarative on the client, GraphQL is declarative on both ends.

### GraphQL queries

GraphQL queries are used to fetch data. They follow this pattern:
...
`$filterProp` defines a new variable that the `field()` lookup uses to filter results to those matching `"value"`. 
...
Similar to a query with arguments, you specify what you're updating and pass-in the update arguments. What's available depends on the implementation of your mutation resolvers on the server.
...
The GraphQL vs. REST debate is a false dichotomy. People like to say GraphQL is *replacing* REST and that's simply not the case. GraphQL is *augmenting* REST.
...
When building a typical CRUD – Create, Read, Update, Delete – API, I like to use 6 questions to help me choose:
...
Use upserts to deal with Create *and* Update. Avoid deletes – update a `hidden` flag instead.

### Should you expose your entire data model verbatim?

No.

Exposing your data model verbatim is a common mistake in API design for both GraphQL and REST. 
...
Exposing server internals like that is a leaky abstraction at best and leads to performance and security issues at worst.

Plus it creates APIs that are hard to use.

There's also the question of derived and computed properties with complex business logic. You don't want to re-implement that on the client using raw data from your database.
