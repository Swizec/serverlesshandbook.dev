...
The second `Resources` must be capitalized, so do `Type`, `Properties`, and `QueueName`. That's because you're dropping a layer below Serverless and writing AWS's native Serverless Application Model (SAM) configuration. It's what the serverless framework compiles configuration do before deploying.
