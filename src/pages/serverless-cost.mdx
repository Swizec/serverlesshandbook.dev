...
A core innovation for serverless is metered pricing – the ability to "scale to zero". You pay what you use and no more.
...
If you're unlucky, the database locks up – [deadlock](https://en.wikipedia.org/wiki/Deadlock). 20ms, 30ms, sometimes minutes pass while your database figures out how to deal.
...
Once you've identified the bottleneck slowing down your system, it's time to fix the issue.

This takes understanding the problem and there is no one solution. Check the [performance chapter](/serverless-performance) for ideas.
...
A good term to start googling is "back pressure" – a way for components to ask others to slow down. Another one is "exponential backoff" where you make a request and if it fails, retry in 1 second. Then 2 seconds. Then 4, and so on.

[SQS from the chapter on queues](/serverless-elements#queue) adds exponential backoff to failing requests.

### Amdahl's law

Another way to think about this is [Amdahl's law](https://en.wikipedia.org/wiki/Amdahl%27s_law) – a formula describing the maximum speed-up from adding resources to a system.

A common problem in [map-reduce style algorithms](/lambda-pipelines) is that you can parallelize the mapping step but not the reduce step. Here's how it works:

1. Split data into units
2. Iterate and modify each unit (map)
3. Collect into final result (reduce)

Each mapping operation depends on 1 unit of data, which makes it easy to do in parallel. The final result depends on *all* data, however, and you can't parallelize that. Or it's very difficult.

Amdahl's law states that no matter how much you improve the optimizable portion of your system, you can never go faster than the bottleneck.

Say it takes 10 seconds to process a dataset. 8 seconds to iterate, 2 seconds to combine. Parallelizing the 80% you can optimize gives you a 3x boost.

![Amdahl's law formula](../images/amdahls-law.png)

Despite 3x better performance, you get a 2.14x improvement on the whole.

### Critical path

Yet a third way to think about the bottleneck problem is the [critical path concept](https://en.wikipedia.org/wiki/Critical_path_method) from project management and [parallel algorithms analysis](https://en.wikipedia.org/wiki/Analysis_of_parallel_algorithms). What is the longest series of sequentially dependent steps in your task?

That series defines the shortest possible execution time. And is a great target for optimization.

For example: A user signs up. You want to store their data, send a welcome email, charge their credit card, and enable access.

What's the fastest you can process their request?
...
You're down to saving data and granting access. That's a couple database requests. Hella fast 🤘

And you can further optimize by doing it in fewer SQL queries.
...
In conclusion, serverless cost optimization is about fine-tuning your system as a whole. Identify the bottlenecks and help them work faster or find ways to avoid them.
