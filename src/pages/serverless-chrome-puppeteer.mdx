...
export const description =
  "Build advanced browser automations with Chrome Puppeteer and serverless AWS Lambda. Take screenshots, test websites, scrape content. Anything a browser can do ✌️"
...
Our example returns that image directly, but you might want to upload to S3 instead and return a URL. _Do not use AWS Lambda to serve images in production._
...
We set up an `imagePath` in `/tmp`. This lets us write to a file on Lambda's hard drive, _but it will not stay there._ As soon as the lambda turns off, the file is gone.
