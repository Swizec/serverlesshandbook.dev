...
In this section I'll share and explain the important code, you can [see the full example on GitHub](https://github.com/Swizec/serverlesshandbook.dev/tree/master/examples/serverless-auth-example). Use [this CodeSandbox app](https://codesandbox.io/s/serverless-auth-example-9ipfb) to try it out.

https://codesandbox.io/s/serverless-auth-example-9ipfb

You can use it to test your implementation. Just replace the Lambda base URL with yours 😊

### The auth flow

The difference between auth for an API and traditional auth comes down to how the client and server talk to each other. Traditional auth uses cookies, API auth relies on JSON Web Tokens (JWT).
