...
The logical next step to platform as a service, which came from The Cloud, which came from virtual private servers, which came from collocation, which came from a computer on your desk running a web server. 🤯
...
Collocation solved physical problems, but not the fact that your servers are bored to tears.
...
The first type of virtualization was pretty basic: [virtual hosts](https://en.wikipedia.org/wiki/Virtual_hosting) let us host multiple websites on the same machine. A domain maps to a specific application on your computer, your web server figures out the mapping, and voila sites can share resources.

Worked pretty great but caused problems. Since websites are so close together on the same computer, you could hack one site to gain access to another. They'd sometimes starve each other for resources and so on.

Real [virtualization](https://en.wikipedia.org/wiki/Virtual_private_server) and later [containerization](https://en.wikipedia.org/wiki/OS-level_virtualisation) were the solution to that problem. Rather than host multiple websites on the same machine, you can host *multiple whole computers* on the same machine. It's like having many brains.
...
You're on the hook for software setup and you share the same machine with who knows how many other users.
...
Once your traffic started to grow, you'd soon need more servers to handle the load. A single server can only handle so many connections, so many application cycles, so much data, etc.

But how do you ensure all your servers are the same? How do you spin them up fast enough when traffic spikes on Black Friday?
...
To set up a new server you often start with a core image from your cloud provider. Has some basic setup and common defaults. You add your tweaks and create a new image.

The cloud then gives you easy controls to create as many instances of that server as you'd like. Press a button and a server shows up. ✨

Sometimes you can even make it scale automatically. Scripts notice traffic rising and quickly spin up some servers. When the traffic subsides, those same scripts can tear the servers back down.
...
The idea behind PaaS is that you pay somebody else to deal with cloud stuff and you focus on your code. They configure your servers and your dockers and your kubernetes and make sure everything plays together. You build the application.

`git push` to deploy and voila, a webapp.

With most PaaS providers you can drop down a few levels if you want. They provide escape hatches to perform low level configuration, let you mess around with databases, and break things.

While PaaS takes care of your servers, you often still have to take care of the "frontend". Set up your domains and DNS config, make your application run just right so the platform is happy, configure your own [CDN](https://en.wikipedia.org/wiki/Content_delivery_network), deal with static files, and so on.

The platform does only servers.

### Serverless is born

[Serverless](https://en.wikipedia.org/wiki/Serverless_computing) comes as the logical step after PaaS. Once you have a system that can use containers to automatically scale and tear down based on demand, use idiomatic configuration for replicability, and painless deploys ... that's almost serverless already.

Serverless's main innovation are _extremely tiny_ containers and the ecosystem of services working together. Server containers are small enough to spin up and tear down every few seconds, and the ecosystem provides all ancillary services you need like static file hosting, queues, and databases.

Server containers can be so small because the code they run is:

1. Very small
2. Extremely standardized
3. Does very little

A serverless "server" is often just a single function responding to a single API endpoint. Request comes in, server wakes up, runs for a few milliseconds, and goes back to bed.
...
We're going to use open source technologies and deploy on AWS Lambda. You can learn about other providers in the [Serverless Flavors](/serverless-flavors) chapter.
...
<video src="../images/start-serverless.mp4" muted autoplay controls width="100%" />
...
In the future we'll write backends using TypeScript, but one thing at a time :)

### Configure your first server

As mentioned, configuration for your server goes in `serverless.yml`. We're going to tell the Serverless framework that we want to use AWS, run nodejs, and that this is a dev project.
...
We're calling our service `hello-world` and specifying details about our provider. The `stage` helps us tell the difference between development, QA, and production deployments. More on that in the [Dev, QA, and prod](/dev-qa-prod) chapter.
...
We started a `functions` section. Each entry here represents its own tiny server – a serverless lambda. Together, they represent the `hello-world` service.
...
Backend functions in a serverless environment look a lot like the JavaScript functions you're used to. They accept some arguments and return a response.
...
It's an async function that accepts some triggering event and returns a response. A success status with a `Hello 👋` body.
...
<video src="../images/deploy-serverless.mp4" muted autoplay controls width="100%" />

And your server is up.

You get a URL to hit your lambda and some debugging output so you know what's going on. My URL is `https://z7pc0lqnw9.execute-api.us-east-1.amazonaws.com/dev/hello`, if you open it in your browser it's gonna say `Hello 👋`

I'll keep it up because I'm not paying for it if nobody clicks. And even when they do, current AWS pricing gives me 1,000,000 clicks for free 😛

### What you got

A lot happened behind the scenes to make this work. The Serverless framework talked to AWS and configured many things.
...
Next, we're going to talk about some pros & cons of using serverless in your next project.